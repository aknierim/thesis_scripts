...
from pyvisgen.simulation.visibiliy import vis_loop
...
class BaselinePlotter:
    ...
    def __call__(self):
        print(f"""
            ...
            Observation start: {self.obs_times[0].iso:>31}
            Observation end: {self.obs_times[-1].iso:>33}
    ...
    def _num_base(self, layout):
        return len(self.array_layout) ** 2 - len(layout)
    ...
    def _observe(self):
        ...
        if (self.sky_image) and ("S" in self.subplots):
            ...
            uu = self.vis.u
            vv = self.vis.v
            obstimes = Time(self.vis.date, format="jd")

        else:
            valid_subset = obs.baselines.get_valid_subset(obs.num_baselines, obs.device)
            obstimes = Time(valid_subset.date, format="jd")
    ...
    def init_plot(self, sampled_color: str = "#111e30") -> None:
        ...
        if "E" in self.subplots:
            ...
            self.ax["E"].add_feature(
                Nightshade(self.obs.obstimes[0].datetime, alpha=0.2)
            )

            self.ax["E"].set_title(
                f"{self.obs.obstimes[0].iso} (UTC)",
        ...
        if "V" in self.subplots:
            ...
            self.scat3 = self.ax["V"].scatter(-u[0], -v[0], s=10, c=self.sampled_color)
            self.scat4 = self.ax["V"].scatter(-u[0], -v[0], s=10, c="#94ab71")

            xlim = np.abs(u.min() + 0.1 * u.min())
            ylim = np.abs(v.min() + 0.1 * v.min())
    ...
    def _update(self, i):
        ...
        if "V" in self.subplots:
            ...
            if i > 0:
                ...
                self.scat3.remove()
                self.scat1 = self.ax["V"].scatter(
                    u[: i - 1], v[: i - 1], s=10, c=self.sampled_color
                )
            ...
            self.scat2.remove()
            self.scat4.remove()
            self.scat2 = self.ax["V"].scatter(u[i], v[i], s=10, c="#94ab71")
            self.scat4 = self.ax["V"].scatter(-u[i], -v[i], s=10, c="#94ab71")
